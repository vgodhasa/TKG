...
public class ModesDictionary {
    ...
    private void parseInvalidSpec(Element modes) throws IOException {
        ...
        if (arg.getSpec().toLowerCase().contains("zos")) {
            reader = new BufferedReader(new FileReader(ottawaCsv), Charset.forName("IBM-1047"));
        } else {
            reader = new BufferedReader(new FileReader(ottawaCsv));
        }
            ...
            if (lineNum++ == 1) {
                ...
                for (int i = 0; i < modesNodes.getLength(); i++) {
                    ...
                    if (mode.getAttribute("number").equals(modeNum)) {
                        ...
                        List<String> filteredInvalidSpecs = new ArrayList<String>();
                        for (String spec : invalidSpecs) {
                            if (!Constants.INGORESPECS.contains(spec)) {
                                filteredInvalidSpecs.add(spec);
                            }
                        }
                        invalidSpecs = filteredInvalidSpecs;
